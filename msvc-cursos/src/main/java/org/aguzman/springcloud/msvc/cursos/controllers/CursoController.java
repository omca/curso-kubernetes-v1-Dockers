...
public class CursoController {
    ...
    @PostMapping("/registrar-usuario-a-curso/{cursoId}")
    public ResponseEntity<?> registrarUsuarioACurso(@RequestBody Usuario usuario, @PathVariable Long cursoId) {
        Optional<Usuario> o;
        try {
    ...
    public ResponseEntity<?> eliminarUsuario(@RequestBody Usuario usuario, @PathVariable Long cursoId) {
        Optional<Usuario> o;
        try {
            o = service.eliminarUsuarioFromCurso(usuario, cursoId);
        } catch (FeignException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Collections.singletonMap("mensaje", "No existe el usuario por " +
                            "el id o error en la comunicacion: " + e.getMessage()));
    ...
    @DeleteMapping("/eliminar-curso-usuario-by-usuarioid/{idUsuario}")
    public ResponseEntity<?> eliminarCursoUsuarioPorId(@PathVariable Long idUsuario){
        service.eliminarCursoUsuarioPorId(idUsuario);
        return ResponseEntity.noContent().build();
    }
